  ...
  _.each(actionType, (item) => {
    ...
    defaultHandler[`${item}_FULFILLED`] = (state, action) => {
      return {
        ...
        meta:   {
          total: action.payload.meta && action.payload.meta.total,
          perPage: action.payload.meta && action.payload.meta.per_page,
          page: action.payload.meta && action.payload.meta.page
        }
      };
    };
